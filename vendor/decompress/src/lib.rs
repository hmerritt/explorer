...
pub enum ObserveEvent<'a> {
    BackendInit,
    EntryStart { path: &'a Path, is_directory: bool },
    EntryComplete { path: &'a Path, bytes: u64, is_directory: bool },
    DirectoryCreate,
    FileCreate,
    MetadataOperation,
    OutputWrite { bytes: u64, elapsed: Duration },
    Flush,
}

pub trait Observer: Send + Sync {
    fn observe(&self, event: ObserveEvent<'_>);
}
...
impl Observer for NoopObserver {
    fn observe(&self, _event: ObserveEvent<'_>) {}
}
...
pub struct ExtractOpts {
    ...
    #[builder(setter(custom), default = "Arc::new(NoopObserver)")]
...
mod tests {
    ...
    #[test]
    fn extract_options_accept_optional_observer_without_changing_defaults() {
