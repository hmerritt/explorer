...
pub(super) fn format_address_path(path: &Path, slash: AddressSlash) -> String {
    if let Some(address) = super::remote_fs::display_address(path) {
        return address;
    }
...
fn resolve_address_input_with_env(
    ...
    if let Some(current) = super::remote_fs::RemoteLocation::from_provider(current_path) {
        if !cleaned.is_empty()
            && !cleaned.contains("://")
            && !(cfg!(windows) && Path::new(&cleaned).is_absolute())
        {
            let address = current.address();
            let base =
                gpui::http_client::Url::parse(&(address.trim_end_matches('/').to_owned() + "/"))
                    .map_err(|e| e.to_string())?;
            return super::remote_fs::RemoteLocation::parse(
                base.join(&cleaned).map_err(|e| e.to_string())?.as_str(),
            )
            .map(|l| l.provider_path());
        }
    }
    if cleaned.is_empty() {
...
fn folder_suggestions_for_input_with_env(
    ...
    if input.starts_with("sftp://") || super::remote_fs::is_remote(current_path) {
        return Vec::new();
    }
